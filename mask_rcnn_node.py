...
class MaskRCNNNode(object):
    ...
    def _build_result_msg(self, msg, result):
        ...
        for i, (y1, x1, y2, x2) in enumerate(result['rois']):
            ...
            f.write( "box\n"+ str(box.x_offset) + " " +str(box.y_offset) + " " + str(box.height) + " " + str(box.width) + "\n"  )
            ...
            mask.data = (result['masks'][:, :, i] * class_id).tobytes()
            result_msg.masks.append(mask)
            f.write( "mask\n"+ str(mask.header) + " " +str( mask.height) + " " + str(mask.width) + "\nmask data:\n" + str(mask.data) + "\n"  )
            f.write("----------------")
            f.flush()
